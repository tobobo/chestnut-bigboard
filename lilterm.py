...
class Terminal:
    ...
    def dumps(self):
        ...
        return ((cursor.x, cursor.y), lines)

shell_exited = False
def open_terminal(command="bash", columns=12, lines=5):
    p_pid, master_fd = pty.fork()
    if p_pid == 0:  # Child.
        argv = shlex.split(command)
        env = dict(TERM="sh", LC_ALL="en_US.UTF-8",
                   COLUMNS=str(columns), LINES=str(lines), PS1="$ ")
...
async def start():
  ...
  outfile = open("pyte.log", "a")
  def print_dump():
    _, lines = terminal.dumps()
    for i, line in lines:
      line_str = ''.join([char for char, _, _, _ in line])
